...
function sendAudioToServer() {
  if (audioChunks.length > 0) {
    ...
    fetch("https://192.168.57.114:5000/transcribe", {
      // HTTPS로 변경
      method: "POST",
      body: formData,
    })
      ...
      .then((data) => {
        document.getElementById("transcript").textContent =
          "질문: " + data.text;
        return fetch("https://192.168.57.114:5000/ask", {
          // HTTPS로 변경
          method: "POST",
...
function handleServerResponse(data) {
  if (data && data.message) {
    ...
    console.error("Received invalid response data:", data);
    document.getElementById("response").textContent = "오류가 발생했습니다.";
  }
}
...
function speak(message) {
  ...
  document.getElementById("response").style.fontSize =
    detectedAge >= 65 ? "4em" : "2em";
...
function captureFrame(stream) {
  ...
  fetch("https://192.168.57.114:5000/analyze_frame", {
    // HTTPS로 변경
    method: "POST",
    headers: { "Content-Type": "application/json" },
